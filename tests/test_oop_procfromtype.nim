...
test "define proc from type":
  ...
  proc TEST_FUNC {.wash: TEST_FUNC_TY.} =
...
test "define lambda from type":
  ...
  let lambda = proc {.wash: TEST_LAMBDA_TY.} = int_val * 2
  check lambda(2) == 4

# TEST_FUNC_TY.genPublicProcAs TEST_FUNC:
proc TEST_PUBLIC_FUNC* {.wash: TEST_FUNC_TY.} =
  debugEcho "HI!"

test "define with additional pragmas":
  proc TEST_FUNC {.wash: TEST_FUNC_TY, TEST_PRAGMA.} =
    discard
  check TEST_FUNC.hasCustomPragma(TEST_PRAGMA)

test "define proc from object nested":
  var PROCEDURE1_result: int
  proc PROCEDURE1 {.wash: TEST_OBJECT.PROCEDURE1.} =
    PROCEDURE1_result = int_val * 2

  proc PROCEDURE2 {.wash: TEST_OBJECT.PROCEDURE2.} =
    int_val * 2
